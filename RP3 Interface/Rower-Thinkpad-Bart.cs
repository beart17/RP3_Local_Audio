...
namespace RP3_Interface
{
    ...
    public class Rower
    {
        ...
        string audioFilePath = "C:\\Users\\bartb\\OneDrive - University of Twente\\Documenten\\University\\Module 11\\GP - Rowing Reimagined\\B4.2.wav";
        ...
        private void ModifyAudioFragment()
        {
            using (var audioFile = new WaveFileReader(audioFilePath)) // Load audio file
            {
                using (var inputStream = new WaveChannel32(audioFile))  // Convert to 32-bit float
                {
                    ...
                    // Check the input is in stereo 16 bit format.
                    if (audioFile.WaveFormat.Encoding != WaveFormatEncoding.Pcm || audioFile.WaveFormat.BitsPerSample != 16 || audioFile.WaveFormat.Channels != 2)
                    {
                        var conversionStream = new WaveFormatConversionStream(new WaveFormat(44100, 16, 2), audioFile);
                    ...
                    Process(processor, audioFilePath, outputFile);


                    /*// Create a SoundTouchWaveStream with the modified audio
                    using (var processStream = new SoundTouchWaveStream((WaveStream)waveProvider, processor))
                    {
